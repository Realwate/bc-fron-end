<template>
  <div class="w" v-loading="isLoading"
    ...
    <div class="main-content">
      ...
      <el-table
        ...
        <el-table-column
          label="用户故事名称"
          width="130"
          prop="nodeinfo.objName">
          <template scope="scope">
            <router-link :to= "config.rootPath+'/document/'+scope.row.nodeinfo.documentid+`?navTitle=${scope.row.nodeinfo.objName}`">
              {{scope.row.nodeinfo.objName}}
            </router-link>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog :close-on-press-escape="false" :close-on-click-modal="false"
               :title="dialogTitle" v-model="dialogVisible" size="small">
      <el-form ref="form" :model="toUpdateRow" label-width="80px">
        <el-row>
          <el-col :span="10">
            <el-form-item label="故事编码">
              <el-input v-model="toUpdateRow.objCode" ></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="10">
            <el-form-item label="故事名称">
              <el-input v-model="toUpdateRow.objName" ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="10" :offset="2">
  ...
  export default {
    data() {
      return {
        ...
        store:null,
        rootNode:null,
        currentProductId:31,//默认用户故事菜单对应31
        config,
        iconSettings:[
    ...
    watch:{
      ...
      nodeInfo(newInfo){
          this.allNodeInfoMap.clear();
          for(let info of newInfo){
              if(this.allNodeInfoMap.has(info.nodeid)){
                this.allNodeInfoMap.get(info.nodeid).push(info);
                continue;
              }
...
//          }
      ...
      updateTableData(){
        ...
        this.$nextTick(()=>{
          ...
          for(let node of this.clickedNodes){
            let infoArray = this.allNodeInfoMap
              .get(node.data.id);
            if(infoArray == null){
                continue;
            }
      ...
      changeUpdateRow(newRow){//点击编辑更新toUpdateRow
        this.toUpdateRow = {
          ...
          objName:newRow.nodeinfo.objName,
          objCode:newRow.nodeinfo.objCode,
      ...
      updateNodeInfo(){//更新nodeInfo
        var nodeInfo = {
          ...
          objName:this.toUpdateRow.objName,
          objCode:this.toUpdateRow.objCode,
        ...
        if(this.dialogTitle == "编辑"){
          api.updateNodeInfo(nodeInfo)
           ...
           //选中的结点
          nodeInfo.nodeid = this.currentNode.data.id;
          nodeInfo.productid = this.currentProductId;

          api.addNodeInfo(nodeInfo)
            .then(({data})=>{
              if(!data.error){
                //更新后如何增量同步？
                this.updateLocalNodeInfo();
      ...
      deleteRow(index,row){
        Msg.confirm("确认删除吗?").then(()=>{
          ...
          api.deleteNodeInfo(row.nodeinfo.id)
            .then(({data})=>{
              if(!data.error){
                this.updateLocalNodeInfo();
              }
